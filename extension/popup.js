// Feeling Forecast Extension - Popup Script
console.log("Feeling Forecast extension loaded");
...
function detectLanguage() {
    // Since extensions can't always access ipapi, use browser's language
    const browserLang = navigator.language || navigator.userLanguage || "en";
    return Promise.resolve(browserLang.split('-')[0].toLowerCase());
}
...
function getWeather(params = {}) {
    ...
    [widgetWeather, widgetMood, widgetLocation].forEach(el => {
    ...
    fetch(apiUrl)
        ...
        .catch(error => {
            ...
            widgetMood.textContent = "Can't calculate mood";
...
async function initPopup() {
    try {
        ...
        document.querySelectorAll('#popup-container > div:not(.popup-footer)').forEach(el => {
            el.classList.add('loading');
        });
        
        // Get user's language and display greeting
...
// Start the popup when DOM is loaded
